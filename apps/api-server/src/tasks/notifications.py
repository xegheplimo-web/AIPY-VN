...
@celery_app.task(
    name="src.tasks.notifications.send_order_notification",
    bind=True,
    max_retries=3
)
...
@celery_app.task(
    name="src.tasks.notifications.send_promotion_notification",
    bind=True,
    max_retries=3
)
...
@celery_app.task(
    name="src.tasks.notifications.send_bulk_notifications",
    bind=True,
    max_retries=3
)
def send_bulk_notifications(
    ...
    try:
        ...
        if not tokens:
            logger.warning("No device tokens found for bulk notification")
            return {"success": 0, "failure": len(user_ids)}
        
        firebase_service = get_firebase_service()
        
        if not firebase_service.is_ready():
            logger.warning("Firebase not ready, cannot send bulk notification")
            return {"success": 0, "failure": len(tokens)}
        
        result = firebase_service.send_to_devices(tokens, title, body, data)
