...
class AdvancedSearchService:
    ...
    def __init__(self):
        self.supported_operators = {
            ...
            "is_null": lambda field, value: (
                field.is_(None) if value else field.isnot(None)
            ),
        }
    ...
    def _filter_by_distance(
        self, stores: List[Store], lat: float, lng: float, radius_km: float
    ) -> List[Store]:
    ...
    async def get_search_suggestions(self, query: str, limit: int = 5) -> List[str]:
        ...
        async with async_session() as session:
            ...
            # Search store names
            store_stmt = (
                select(Store.name).where(Store.name.ilike(f"%{query}%")).limit(limit)
            )
